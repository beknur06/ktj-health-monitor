...
/**
 * Нормализация параметров в deviation 0..1.
 * 0 = внутри нормы, 1 = критическое отклонение.
 *
 * Дублирует логику processor-service намеренно:
 * health-index-service должен работать автономно,
 * даже если processor не отправил нормализованные значения.
 */
@Service
public class DeviationCalculator {
    ...
    private static final Map<String, Range> RANGES = Map.ofEntries(
        ...
        Map.entry("fuel_level",             new Range(600, 6000, 300, 6500)),
    ...
    /**
     * Рассчитать deviation (0..1) для параметра.
     * Возвращает -1 если параметр неизвестен.
     */
